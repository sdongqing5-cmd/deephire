"""Department Interview API endpoints - 部门面试接口"""
...
class ScheduleDepartmentInterviewRequest(BaseModel):
    """安排部门面试请求"""
    application_id: str
    interviewer_id: str
...
class CompleteDepartmentInterviewRequest(BaseModel):
    """完成部门面试请求"""
...
async def schedule_department_interview(
    request: ScheduleDepartmentInterviewRequest,
    ...
    try:
        ...
        interview = await DepartmentInterviewService.schedule_interview(
            db=db,
...
async def start_department_interview(
    interview_id: str,
    db: Session = Depends(get_db)
):
    """开始部门面试"""
    try:
        ...
        interview = await DepartmentInterviewService.start_interview(
            db=db,
...
async def complete_department_interview(
    interview_id: str,
    request: CompleteDepartmentInterviewRequest,
    db: Session = Depends(get_db)
):
    """完成部门面试并填写评价表"""
    try:
        ...
        interview = await DepartmentInterviewService.complete_interview(
            db=db,
            interview_id=interview_id,
            interviewer_id=interviewer_id,
            interviewer_name=interviewer_name,
...
async def get_department_interview(
    interview_id: str,
    db: Session = Depends(get_db)
):
    """获取部门面试详情"""
    try:
        interview = DepartmentInterviewService.get_by_id(db=db, interview_id=interview_id)
        if not interview:
...
async def get_department_interviews_by_application(
    application_id: str,
    db: Session = Depends(get_db)
):
    """获取应聘记录的所有部门面试"""
    try:
        interviews = DepartmentInterviewService.get_by_application(
            db=db,
...
async def cancel_department_interview(
    interview_id: str,
    ...
    """取消部门面试"""
    try:
        interview = DepartmentInterviewService.cancel_interview(
            db=db,
            interview_id=interview_id,
...
async def reschedule_department_interview(
    interview_id: str,
    ...
    """改期部门面试"""
    try:
        interview = DepartmentInterviewService.reschedule_interview(
            db=db,
            interview_id=interview_id,
...
@router.get("/interviewer/{interviewer_id}/schedule", response_model=dict)
async def get_interviewer_schedule(
    ...
    """获取面试官的面试日程"""
    try:
        interviews = DepartmentInterviewService.get_interviewer_schedule(
            db=db,
            interviewer_id=interviewer_id,
